...
public class DatasetService {
     ...
     * There is room for optimization as for cases where a bounding box is provided, two
     * Mongo queries are performed: one to search the IDs of the datasets in the grids collection
     * and other to recover the complete information of the dataset by using the previously
     * recovered ID
     *
     * @param workspaceName
    ...
    public Flux<DatasetDTO> getDatasets(String workspaceName,
        ...
        if (southWest.isEmpty() && northEast.isEmpty()) {
            return datasetRepositoryInterface
                    .findByWorkspaceNameAndTimeWindow(workspaceName, fromDate, toDate)
                    .map(DatasetDTO::new);
        }

        //Case 2: Bounding box specified
        //Parse UTMCoords from Strings
        UTMCoord UTMCoordSouthWest = southWest.isEmpty() ? UTMCoord.defaultSouthWest() : new UTMCoord(southWest);
        UTMCoord UTMCoordNorthEast = northEast.isEmpty() ? UTMCoord.defaultNorthEast() : new UTMCoord(northEast);
    ...
    public Mono<DatasetDTO> addDataset(String workspaceName, DatasetDTO datasetDTO) {
        ...
        return Mono.zip(datasetRepositoryInterface.save(dataset), workspaceMono)
                .flatMap(tuple -> {
                    if (zoneSouthWestBottom.equals(zoneNorthEastTop)) {
                        ...
                        return saveGrids(gridCells, dataset, workspaceName)
                        ...
                        return gridsNECoord.concatWith(gridsSWCoord).then(Mono.just(tuple.getT1()));
                    }
                })
    ...
    /**
     * Store grid cells that collide with a dataset bounding box
     *
     * @param grids
     * @param dataset
     * @param workspaceName
    ...
    public Mono<Boolean> datasetHasDataAssociated(String workspaceName, String datasetName) {
        ...
        return datasetRepositoryInterface.findByWorkspaceAndDataset(workspaceName, datasetName)
                .map(Dataset::getDataAssociated).map(state -> {
                    switch (state){
                        case NO_DATA:
                        case ERROR_ON_INSERTION:
                            return false;
                        default:
                            return true;
                    }
